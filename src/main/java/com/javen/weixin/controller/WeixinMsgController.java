...
import com.javen.model.MemberBarron800;;
...
public class WeixinMsgController extends MsgControllerAdapter {
	...
	protected void processInTextMsg(InTextMsg inTextMsg)
	{
		...
		if ("help".equalsIgnoreCase(msgContent) || "帮助".equals(msgContent)) {
			...
			render(outMsg);
		}else if (msgContent.equals("连连看")){
		     OutNewsMsg outMsg = new OutNewsMsg(inTextMsg);
		     String strUrl = "http://www.newsater.org/apex?openid=" + inTextMsg.getFromUserName();
            outMsg.addNews("APEX召唤授 单词连连看", "巴郎800单词系列", "http://www.newsater.org/weixin/images/barron800.jpg", strUrl);
            render(outMsg);
		}else if (msgContent.equals("获取成绩")){
			MemberBarron800 memBar = new MemberBarron800();
			List<MemberBarron800> memBarl = memBar.getTotalStatisticData();
			String str = "";
			str = "总局数：" + memBarl.get(0).getLong("Fnum") + "  总时长：" + memBarl.get(0).getBigDecimal("FsumTime");
			OutTextMsg outMsg = new OutTextMsg(inTextMsg);
			outMsg.setContent(str);
			render(outMsg);
		}
		else if(msgContent.equals("数据库")){
			Member member = new Member();
			member.set("Fweixin_id","abv");
			member.set("Fmember_name","Hinge");
			member.set("Fmobile_number","13965178823");
			member.set("Fmember_school", 1);
			member.set("Fmember_age",15);
			member.set("Fmember_gender",1);
			member.set("Fmember_grade",2);
			member.save();
			
			List<Member> memberl = member.getMemberByWeixinId("abv");
			String str = "";
			for (Member mem: memberl){
				str += "姓名：" + mem.getStr("Fmember_name") + "  手机号码：" + mem.getStr("Fmobile_number") + "  年龄：" + mem.getLong("Fmember_age") + "\n";
			}
			OutTextMsg outMsg = new OutTextMsg(inTextMsg);
			outMsg.setContent(str);
			render(outMsg);
